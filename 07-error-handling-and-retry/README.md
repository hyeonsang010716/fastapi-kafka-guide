...
## 0. 이 챕터의 자리 — Resilience 우산 안

이 챕터에서 배우는 retry / backoff / DLQ 는 사실 더 큰 이름의 *한 가족* 입니다. 그 가족 이름이 **Resilience(회복 탄력성)** 예요.

> **Resilience** = 시스템이 장애 상황에서 *버티고, 흡수하고, 회복하는* 능력. 이걸 만드는 패턴들의 묶음.

| 어떤 장애를 다루는가 | 어떤 패턴? | 어디서 다루는가 |
|---------------------|-----------|----------------|
| 일시적 흔들림(잠깐의 네트워크 끊김 등) | Retry, Exponential Backoff | **이 챕터** |
| 영구적 실패 메시지의 격리 | Dead Letter Queue (DLQ) | **이 챕터** |
| 무한 대기로 인한 자원 고갈 | Timeout | 13 챕터 |
| "이미 죽은" 외부 의존으로의 호출 차단 | Circuit Breaker | 13 챕터 |
| 한 컴포넌트 장애가 전체로 번지지 않게 격리 | Bulkhead | 13 챕터 |
| 빠른 생산자 / 느린 소비자의 흐름 제어 | Backpressure | (소개) |

이 챕터는 우산의 *왼쪽 절반* — "일시적 장애를 흡수하기" + "영구 실패를 격리하기" 를 다룹니다. 13 챕터가 *오른쪽 절반* — "장애가 시스템 전체로 번지지 않게 차단하기" 를 이어받습니다. 두 챕터를 같이 읽으면 resilience 패턴 가족이 한 화면에 정리돼요.
